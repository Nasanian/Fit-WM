...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
  int ismax, iscentered, wasfloating, isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, isterminal, noswallow, snapstatus;
	pid_t pid;
	Client *next;
	Client *snext;
	Monitor *mon;
...
static void detachstack(Client *c);
static void redrawwin(const Arg *arg);
...
static void updategapstatus(void);
...
movemouse(const Arg *arg)
{
	...
	do {
		...
		case MapRequest:
			...
			break;
		case MotionNotify:
			if ((ev.xmotion.time - lasttime) <= (1000 / xfps))
				continue;
...
togglefloating(const Arg *arg)
{
	...
	if (selmon->sel->isfloating){
		resize(selmon->sel, selmon->sel->x, selmon->sel->y,
			selmon->sel->w, selmon->sel->h, 0);
		animateclient(selmon->sel, selmon->sel->x, selmon->sel->y,
		       selmon->sel->w, selmon->sel->h, 7, 0);
  }
	arrange(selmon);
}
...
updatestatus(void)
{
	...
	strcpy(stext, "Nasa-WM");
	drawbar(selmon);
}

/*
void
updategapstatus(void)
{
	if (!gettextprop(root, XA_WM_NAME, gtext, sizeof(gtext)))
	strcpy(gtext, gappx);
}
*/
...
{
	unsigned int i, n, h, mw, mx, my, oty, ety, tw, ty;
	Client *c;
